...
if( !function_exists('noo_navbar_btn') ) :
	function noo_navbar_btn ( $items, $args ) {
		if( !noo_get_option('noo_header_nav_post_btn', true) ) {
			return $items;
		}
		if ($args->theme_location == 'primary') {
			...
			if( Noo_Member::is_employer() || ( !Noo_Member::is_candidate() && $default_button == 'post_job' ) || !jm_resume_enabled() ) {
				...
				$items .= apply_filters('noo_post_job_btn', '<li id="nav-menu-item-post-btn" class="menu-item-post-btn"><a href="' . $link . '">' . jm_get_button_text() . '</a></li>' );
			} else {
				$link = Noo_Member::get_post_resume_url();
	        	$items .= apply_filters('noo_post_resume_btn', '<li id="nav-menu-item-post-btn" class="menu-item-post-btn"><a href="' . $link . '">' . jm_get_button_text('resume') . '</a></li>' );
			}
	    }
