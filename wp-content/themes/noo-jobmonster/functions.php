...
function jobmonster_maybe_start_session() {
	if(!session_id() && !is_admin()) {
		session_start();
	}
}
add_action('init', 'jobmonster_maybe_start_session', 1);
...
require_once NOO_FRAMEWORK_ADMIN . '/noo_resume_alert.php';
...
require_once NOO_FRAMEWORK . '/add-ons/social-login/class-linkedin-apply.php';
...
if(!function_exists('smk_sidebar_version')){
	if (!defined('SMK_SIDEBAR_GENERATOR_DIR')) {
		define('SMK_SIDEBAR_GENERATOR_DIR', NOO_FRAMEWORK_ADMIN . '/smk-sidebar-generator/');
	}
	if (!defined('SMK_SIDEBAR_GENERATOR_URL')) {
		define('SMK_SIDEBAR_GENERATOR_URI', NOO_FRAMEWORK_ADMIN_URI . '/smk-sidebar-generator/');
	}
	
	require_once SMK_SIDEBAR_GENERATOR_DIR . 'smk-sidebar-generator.php';
}
// Visual Composer
