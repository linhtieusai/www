...
switch ($action){
	...
	break;
	case 'post_job':
		if('yes' === jm_get_job_setting( 'job_require_company_profile','') && noo_check_company_empty_require_field() ){
			noo_message_add(__('Please update Company Information require field before post a job','noo'),'error');
			jm_force_redirect(Noo_Member::get_company_profile_url());
		}
		if( !jm_can_post_job() ) {
			noo_message_add(__('You can not add job','noo'),'error');
			if(jm_check_package_post_job() && Noo_Member::is_employer()){
                jm_force_redirect(Noo_Member::get_endpoint_url('manage-plan'));
			}else{
		...
		<div id="step_content_form" class="jstep-content">
			...
			<div class="form-actions form-group text-center clearfix">
				<?php if( jm_check_package_post_job() && !empty( $package_id ) ): ?>
	...
	case 'preview_job':
		...
		if(!jm_can_post_job() && jm_check_package_post_job()){
